...
public:
  ...
  void stepExecute(int &cycles, Memory &mem);
...
void X85::stepExecute(int &cycles, Memory &mem) {
  while (true) {
    auto lastInstructionLocation = PC;
    auto instruction = fetchByte(cycles, mem);
...
void X85::stepPrint(Memory &mem) {
  resetProgramCounter();
  int cycles = 0;
  stepExecute(cycles,mem);
  std::cout << " Number of cycles : " << cycles << std::endl;
}
